...
process_wait (tid_t child_tid) 
{
  ...
  if(child_tid == NULL)
    return -1; //tid not valid

  for (e = list_begin (&cur->children); e != list_end (&cur->children); e = list_next (e))
  { 
    //is the child_tid associated with this thread?
    if(list_entry(e, struct wait_status, elem)->tid == child_tid)
      break; //yes, yes it is.
  }

  if(e == list_end(&cur->children)) 
    return -1; //child not associated with this thread
  
  if(!list_empty(&list_entry(e, struct wait_status, elem)->dead.waiters))
    return -1; //Another thread is already waiting on this

  sema_down(&list_entry(e, struct wait_status, elem)->dead); //wait for thread to die
  return &list_entry(e, struct wait_status, elem)->exit_code;
}
